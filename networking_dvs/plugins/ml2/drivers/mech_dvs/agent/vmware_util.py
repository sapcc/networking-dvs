...
class VMWareUtil():
    def __init__(self, config=None):
        ...
        self.dvs_ref = self._get_dvs(self.dvs_name)
        self.dvpg = self._get_dvpg_by_name(self.portgroup_name)
    ...
    def get_ports_on_dvpg(self, default_vlan_only=True):
        ...
        for p in ports:
            ...
            if hasattr(p, "config") \
                ...
                if segmentation_id == self.default_vlan:
                    if hasattr(p.config, "name") and p.config.name:
                        ...
                        reset_port_info_specs.append(self._get_empty_port_config_spec(p))
            ...
            if hasattr(p, "state") \
                ...
                mac = self._get_vm_mac(p.connectee.connectedEntity.value, p.connectee.nicKey)
    ...
    def _get_vm_mac(self, vm_name, nic_key):
        start = time.clock()

        vm_props = self._get_vm_properties(vm_name, ["name", "config.hardware"])
    ...
    def _get_vm_properties(self, vm_key, properties):
        vms = self._session.invoke_api(vim_util, "get_objects", self._session.vim,
    ...
    def _get_dvpg_by_name(self, dvpg_name):
